...
from .data import DEFAULT_PRICE_STORE, build_evidence, load_price_store
...
def resolve_evidence(
    ...
    fundamentals = (
        json.loads(fundamentals_file.read_text(encoding="utf-8"))
        if fundamentals_file.exists() else None
    )
    macro_file = processed / "macro.json"
    macro_store = (
        json.loads(macro_file.read_text(encoding="utf-8")) if macro_file.exists() else None
    )
