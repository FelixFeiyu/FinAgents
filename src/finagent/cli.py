    ...
    if args.command == "ingest":
        ...
        if args.dataset in {"fundamentals", "all"}:
            if args.source == "synthetic":
                fundamentals = synthetic_fundamentals(universe.symbols)
                save_fundamentals_store(fundamentals)
            else:
        ...
        if args.dataset in {"macro", "all"}:
            if args.source == "synthetic":
                macro = synthetic_macro(weeks=args.weeks)
                save_macro_store(macro)
            else:
                macro = ingest_macro()
            summary["macro"] = {ticker: len(series) for ticker, series in macro.items()}
        manifest = write_dataset_manifest("data/processed", universe.symbols)
